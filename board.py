...
class Board:
    ...
    def insertChar(self, c, pos):
        x,y = pos
        self.board[y][x] = (c, self.board[y][x][1])

    def insertWord(self, w, pos, orientation):
        if self.score(w, pos, orientation) > -1:
            x,y = pos
            if orientation == 'h':
                for i in xrange(len(w)): 
                    self.insertChar(w[i], (x+i,y))
            elif orientation == 'v':
                for i in xrange(len(w)): 
                    self.insertChar(w[i], (x,y+i))
            else:
                raise Exception("Invalid orientation: %s"%orientation)
        else:
            raise Exception("Invalid insertion: {word: %s, pos: %s, orient: %s"%(w, pos, orientation))
