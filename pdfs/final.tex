...
The 'vanilla' version of our code wins only 11% of the time, with an average score differential of 407 points, in Quackle's favor. It performs slightly better with rack heuristics, although the difference is barely statistically significant.  The smae is true for a monte carlo-enabled AI, although the change in mean score delta is more notably improved. We didn't have enough time to iterate through an extensive training set for our monte carlo + rack heuristic weight vector, so it is not terrible surprising that this performed poorly. In general this data indicates that some of the features we decided not to impliment were more important than we had anticipated (end game analysis and managment of wild squares, decided when to exchange tiles, etc). 
